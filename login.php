...
<style>
/* ═══════════════════════════════════════
   TOKENS — identical to home & front end
═══════════════════════════════════════ */
...
:root{
  ...
  --shadow-md: 0 8px 32px rgba(45,48,40,0.10);
  --shadow-lg: 0 20px 60px rgba(45,48,40,0.13);
}
...
.bg-ambient{
  ...
  background:
    ...
    radial-gradient(ellipse 35% 32% at 15% 20%,  #f0d8cc 0%, transparent 55%),
    #edf5eb;
}
...
.ptc{position:absolute;opacity:0;animation:ptcDrift linear infinite;will-change:transform}
@keyframes ptcDrift{
...
.nav-logo img{height:clamp(26px,3.2vw,40px);width:auto}
.nav-brand{font-family:var(--ff-d);font-size:clamp(1rem,2.2vw,1.4rem);font-weight:700;color:var(--char)}
.nav-brand .s{font-family:var(--ff-s);font-size:0.68em;color:var(--sage);margin-left:3px}
.nav-home{
  font-size:0.84rem;font-weight:500;color:var(--gray);text-decoration:none;
  display:flex;align-items:center;gap:6px;
  transition:color 0.2s;
}
.nav-home:hover{color:var(--sage-d)}
...
.main{
  ...
  padding:clamp(24px,5vh,60px) 20px;
}
...
.card{
  ...
  box-shadow:var(--shadow-lg), 0 0 0 4px rgba(184,216,188,0.12);
...
.card-stripe{
  height:5px;
  background:linear-gradient(90deg, var(--sage-d) 0%, var(--sage) 45%, var(--terra) 100%);
}

.card-body{
  padding:clamp(28px,5vh,44px) clamp(24px,5vw,40px) clamp(24px,4vh,36px);
}

/* Logo area */
.card-logo{
  display:flex;flex-direction:column;align-items:center;
  gap:6px;margin-bottom:clamp(20px,3.5vh,32px);
  animation:fadeUp 0.6s ease 0.4s both;
}
.card-logo img{
  width:clamp(52px,8vw,72px);height:auto;
  filter:drop-shadow(0 4px 12px rgba(95,143,101,0.2));
}
.card-logo-word{
  font-family:var(--ff-d);
  font-size:clamp(1.3rem,3vw,1.7rem);
  font-weight:700;color:var(--char);letter-spacing:-0.4px;
}
.card-logo-word .g{color:var(--sage-d)}
.card-logo-sub{
  font-size:0.72rem;color:var(--muted);font-weight:400;letter-spacing:0.3px;
}
...
.card-heading em{color:var(--terra);font-style:italic}
.card-sub{
  font-size:0.84rem;color:var(--gray);font-weight:300;line-height:1.6;
...
.form-divider-line{flex:1;height:1px;background:var(--sage-l)}
.form-divider-label{
  font-size:0.65rem;font-weight:500;letter-spacing:2px;
  text-transform:uppercase;color:var(--sage-d);white-space:nowrap;
}
...
.field:nth-child(1){animation-delay:0.65s}
.field:nth-child(2){animation-delay:0.72s}

.field-label{
  display:flex;align-items:center;gap:6px;
  font-size:0.76rem;font-weight:500;color:var(--gray);
  letter-spacing:0.3px;margin-bottom:7px;
}
.field-label svg{width:13px;height:13px;opacity:0.6}
...
.field-input{
  ...
  background:var(--sage-lll);
  font-family:var(--ff-b);
  font-size:0.9rem;color:var(--char);
  outline:none;
  transition:border-color 0.22s, box-shadow 0.22s, background 0.22s;
}
.field-input:focus{
  border-color:var(--sage);
  box-shadow:0 0 0 3px rgba(95,143,101,0.14);
...
.pw-toggle:hover{color:var(--sage-d)}

/* Forgot */
.forgot-row{
  display:flex;justify-content:flex-end;
  margin-top:-6px;margin-bottom:clamp(18px,2.5vh,24px);
  animation:fadeUp 0.6s ease 0.78s both;
}
.forgot-link{
  font-size:0.74rem;color:var(--terra);font-weight:500;
...
.btn-signin{
  ...
  box-shadow:0 6px 20px rgba(58,107,64,0.28);
...
.btn-signin:hover{transform:translateY(-2px) scale(1.01);box-shadow:0 10px 28px rgba(58,107,64,0.36)}
.btn-signin:active{transform:translateY(1px) scale(0.99);transition:100ms}
...
.btn-skip{
  ...
  animation:fadeUp 0.6s ease 0.88s both;
  margin-bottom:clamp(14px,2vh,20px);
}
.btn-skip:hover{background:var(--sage-lll);border-color:var(--sage);color:var(--sage-d)}
.btn-skip:active{transform:scale(0.98)}

/* Signup link */
.signup-row{
  text-align:center;
  animation:fadeUp 0.6s ease 0.94s both;
}
.signup-row p{font-size:0.8rem;color:var(--muted);font-weight:300}
.signup-link{
  color:var(--terra);font-weight:500;font-size:0.82rem;
  text-decoration:none;display:inline-flex;align-items:center;gap:4px;
  transition:opacity 0.2s;
  margin-top:6px;
}
.signup-link:hover{opacity:0.7;text-decoration:underline}
.signup-link svg{width:13px;height:13px}

/* FOOTER */
footer{
  position:relative;z-index:10;text-align:center;
  padding:clamp(14px,2.5vh,22px) 24px;
...
<main class="main">
  <div class="card">
    ...
    <div class="card-body">
      ...
      <!-- Logo -->
      <div class="card-logo">
        <img src="images/LOGINPICTURE.png" alt="MedMeal" onerror="this.style.display='none'">
        <span class="card-logo-word">Medi<span class="g"> ආහාර</span></span>
      </div>

      <!-- Heading -->
      <h1 class="card-heading">Welcome <em>back!</em></h1>
      <p class="card-sub">Sign in to see your saved recommendations and meal history.</p>

      <!-- Divider -->
      <div class="form-divider">
        <div class="form-divider-line"></div>
        <span class="form-divider-label">Your credentials</span>
        <div class="form-divider-line"></div>
      </div>
      ...
      <form class="signin-form" action="SignIn.php" method="post">
        ...
        <div class="field">
          ...
          <input class="field-input" type="email" id="email" name="email"
                 placeholder="e.g. johnjoe@gmail.com" required>
        </div>
        ...
        <!-- Secondary: Skip -->
        <button class="btn-skip" type="button" onclick="window.location.href='Front_End.php'">
          Continue as guest
        </button>
...
/* Floating particles — same logic as front end */
...
function spawn(){
  ...
  const dy=-(55+Math.random()*40);
  const dx=(Math.random()-0.5)*28;
  const rot=(Math.random()-0.5)*360;
  el.style.cssText=`left:${x}%;bottom:${-5+Math.random()*8}%;--ptc-y:${dy}vh;--ptc-x:${dx}vw;--ptc-r:${rot}deg;animation-duration:${dur}s;animation-delay:${Math.random()*dur}s;font-size:${0.75+Math.random()*0.6}rem`;
  field.appendChild(el);
  setTimeout(()=>el.remove(),(dur+4)*1000);
}
for(let i=0;i<18;i++) setTimeout(spawn, i*900);
